...
const myVideoModel = mongoose.model('MyVideo', MyVideoSchema);
const find = thunkify(function () {
    myVideoModel.find.apply(myVideoModel, arguments)
});
module.exports = {
    findIndex: function (index) {
        return myVideoModel.find({ index: { $gt: index,$lte:index+num},sliceStartTime:null,sliceEndTime:null,videos:{"$ne":[]} }).exec();
    },
    findByIndex: function (index) {
        return myVideoModel.findOne({ index:index }).exec();
    },
    find: function (id) {
